...
public class ArcBall {
    ...
    if (id == MouseEvent.MOUSE_DRAGGED) {
      mouseDragged();
    	...
    	if (event.isControlDown()) 
    		defineFirstPoint(event);
    	else if (event.isAltDown())
    		defineSecondPoint(event);
      mousePressed();
    }
  }
  ...
  public void defineFirstPoint(MouseEvent event) {
	  float min = Float.MAX_VALUE;
	  Vertex<Point_3> selected = null;
	  float clickedX = event.getX();
	  float clickedY = event.getX();
	  for (Vertex<Point_3> v: ((MeshViewer) parent).getSurfaceMesh().polyhedron3D.vertices) {
		  float x = parent.screenX(new Float(v.getPoint().x), new Float(v.getPoint().y), new Float(v.getPoint().z));
		  float y = parent.screenY(new Float(v.getPoint().x), new Float(v.getPoint().y), new Float(v.getPoint().z));
		  if ((clickedX - x)*(clickedX - x) + (clickedY - y)*(clickedY - y) < min)
			  selected = v;
	  }
	  selected.extremum = true;
	  ((MeshViewer) parent).setFirstPoint(selected);
  }
  
  public void defineSecondPoint(MouseEvent event) {
	  float min = Float.MAX_VALUE;
	  Vertex<Point_3> selected = null;
	  float clickedX = event.getX();
	  float clickedY = event.getY();
	  System.out.println("Trying " + clickedX + " " + clickedY);
	  for (Vertex<Point_3> v: ((MeshViewer) parent).getSurfaceMesh().polyhedron3D.vertices) {
		  float x = parent.screenX(new Float(v.getPoint().x*60), new Float(v.getPoint().y*60), new Float(v.getPoint().z)*60);
		  float y = parent.screenY(new Float(v.getPoint().x*60), new Float(v.getPoint().y*60), new Float(v.getPoint().z)*60);
		  System.out.println(x + " " + y);
		  if ((clickedX - x)*(clickedX - x) + (clickedY - y)*(clickedY - y) < min)
			  selected = v;
	  }
	  System.out.println("Tried " + clickedX + " " + clickedY);

	  float x = parent.screenX(new Float(selected.getPoint().x*60), new Float(selected.getPoint().y*60), new Float(selected.getPoint().z)*60);
	  float y = parent.screenY(new Float(selected.getPoint().x*60), new Float(selected.getPoint().y*60), new Float(selected.getPoint().z)*60);
	  System.out.println("Got " + x + " " + y);
	  selected.extremum = true;
	  ((MeshViewer) parent).setSecondPoint(selected);
  }
