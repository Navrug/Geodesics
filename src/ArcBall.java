...
public class ArcBall {
	...
	public void defineFirstPoint(MouseEvent event) {
		...
		float s=(float)((MeshViewer) parent).mesh.scaleFactor;
		...
		for (Vertex<Point_3> v: ((MeshViewer) parent).getSurfaceMesh().polyhedron3D.vertices) {
			float x = parent.screenX(new Float(v.getPoint().x*s), new Float(v.getPoint().y*s), new Float(v.getPoint().z)*s);
			float y = parent.screenY(new Float(v.getPoint().x*s), new Float(v.getPoint().y*s), new Float(v.getPoint().z)*s);
			if ((clickedX - x)*(clickedX - x) + (clickedY - y)*(clickedY - y) < min) {
				selected = v;
				min = (clickedX - x)*(clickedX - x) + (clickedY - y)*(clickedY - y);
			}
		}
